...
class Inshot extends Component {
  constructor(props) {
    ...
    this.swipedCardPosition = new Animated.ValueXY({ x: 0, y: -Height });
    this.state = {
      currentItem: 0,
    };
  }
  componentWillMount() {
    this.PanResponder = PanResponder.create({
      ...
      onPanResponderMove: (e, gesture) => {
        if (gesture.dy > 0 && this.state.currentItem > 0) {
          this.swipedCardPosition.setValue({ x: 0, y: -Height + gesture.dy });
        } else {
          this.position.setValue({ x: 0, y: gesture.dy });
        }
      },
      onPanResponderRelease: (e, g) => {
        if (this.state.currentItem > 0 && g.dy > 50 && g.vy > 0.7) {
          Animated.timing(this.swipedCardPosition, {
            toValue: { x: 0, y: 0 },
            duration: 400,
          }).start(() => {
            this.setState({ currentItem: this.state.currentItem - 1 });
            this.swipedCardPosition.setValue({ x: 0, y: -Height });
          });
        } else if (-g.dy > 50 && -g.vy > 0.7) {
  ...
  renderCards = () => {
    return articles
      .map((item, i) => {
        ...
        } else if (i < this.state.currentItem) {
          return null;
        }
